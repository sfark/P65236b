...
d_hat_BIC <- fracdiff(X_t,nar=1,nma=2)$d #d=6.848853e-05
...
which(result_diffy12[,1] == min(result_diffy12[,1]), arr.ind = TRUE)#AIC vælger, p=5, q=4
which(result_diffy12[,2] == min(result_diffy12[,2]), arr.ind = TRUE)#BIC vælger, p=1, q=2 

#AIC vælger den bedste model til at være en ARfIMA(5,d_hat_BIC,4)
#BIC vælger den bedste mdoel til at være en ARfIMA(1,d_hat_BIC,2)

#AIC og BIC for model valgt ud fra AIC
AIC(arfima(X_t,order = c(5,0,4),fixed = list(frac=d_hat_BIC)))#AIC=14134.85
BIC(arfima(X_t,order = c(5,0,4),fixed = list(frac=d_hat_BIC)))#BIC=14203.16

#AIC og BIC for model valgt ud fra BIC
AIC(arfima(X_t,order = c(1,0,2),fixed = list(frac=d_hat_BIC)))#AIC=14130.11
BIC(arfima(X_t,order = c(1,0,2),fixed = list(frac=d_hat_BIC)))#BIC=14164.27

#der fittes en ARIMA model til DiffY12, SARIMA giver forskellige plots
sarima(DiffY12,5,0,4,details = T,no.constant = T,tol=0.0000000000000000000000000001)
sarima(DiffY12,1,0,2,details = T,no.constant = T)

#Residualerne for de 2 forskellige modeller
res.arima.AIC54 <- residuals(arima(DiffY12,order = c(5,0,4)))#p=5, q=4
res.arima.BIC12 <- residuals(arima(DiffY12,order = c(1,0,2)))#p=1, q=2

fit54 <- fitted.values(arima(DiffY12,order = c(5,0,4),include.mean = F))
fit12 <- fitted.values(arima(DiffY12,order = c(1,0,2),include.mean = F))

#plots, acf og pacf valgt ud fra AIC
plot(acf(res.arima.AIC53)) #p=5, q=4
{
di_ACfF <- acf(res.arima.AIC54 ,plot = FALSE,lag.max = 20)
...
di_pACfF <- pacf(res.arima.AIC54 ,plot = FALSE)
...
acf(res.arima.AIC53)  #Acf'en for residualerne
pacf(res.arima.AIC53) #Pacf for residualerne
...
plot(fitted.values(arima(DiffY12,order = c(5,0,3),include.mean = F)),col="red",ylim=c(-200,230),lty=2)
lines(DiffY12)
...
ggplot(data=DiffY12,mapping = aes(1:length(DiffY12),DiffY12,colour="DiffY12"))+
  ...
  geom_line(aes(y=fit54,colour="fit54"))+
  scale_color_manual(name="Models",labels=c("Original","ARFIMA(1,6.85e-05,2)","ARFIMA(5,6.85e-05,4)"),
                     values = c("DiffY12"="black","fit12"= "red","fit54"="blue"))+
  xlab("Time")+ylab("Sample")

forskel1 <- abs(DiffY12[1:length(DiffY12)])-abs(fit54[1:length(DiffY12)])
...
which(AIC_BIC_Resultat2[,1] == min(AIC_BIC_Resultat2[,1]), arr.ind = TRUE) #AIC, p=8,q=9
which(AIC_BIC_Resultat2[,2] == min(AIC_BIC_Resultat2[,2]), arr.ind = TRUE) #BIC, p=8,q=9

###Estimering af d-parameter 
d_hat_AIC <- fracdiff(X_t,nar = 8,nma = 9)$d

#residualerne for de 2 beste modelle valgt ud fra AIC og BIC
acf(residuals(fracdiff(X_t,nar = 8,nma = 9)))
...
for (i in 0:10) {
  for (j in 0:10) {
    ...
    colnames(result_diffy_AIC) <- names
  }
  if (i==10 &&j==10) {
...
AIC(arfima(X_t,order = c(8,0,9),fixed = list(frac=d_hat_AIC)))
BIC(arfima(X_t,order = c(8,0,9),fixed = list(frac=d_hat_AIC)))


sarima(DiffY_AIC,1,0,2,details = T,no.constant = T)
sarima(DiffY_AIC,5,0,7,details = T,no.constant = T)
}
fit542 <-   fitted.values(arima(DiffY_AIC,order = c(5,0,4)))
...
ggplot(data=DiffY_AIC,mapping = aes(1:length(DiffY_AIC),DiffY_AIC,colour="DiffY_AIC"))+
  ...
  geom_line(aes(y=fit542,colour="fit542"))+
  scale_color_manual(name="Models",labels=c("Original","ARFIMA(1,6.85e-05,2)","ARFIMA(5,6.85e-05,4)"),
                     values = c("DiffY_AIC"="black","fit122"= "red","fit542"="blue"))+
  xlab("Time")+ylab("Sample")



res.arima.AIC <- residuals(arima(DiffY_AIC,order = c(8,0,9),optim.control = list(maxit = 1000)))
...
d_hat_fdGPH <- fdGPH(X_t)$d #d=0.2943173
...
which(result_diff_fdGPH_BIC[,1] == min(result_diff_fdGPH_BIC[,1]), arr.ind = TRUE)#AIC, p=4, q=5
which(result_diff_fdGPH_BIC[,2] == min(result_diff_fdGPH_BIC[,2]), arr.ind = TRUE)#BIC, p=1, q=2

fit453 <- fitted.values(arima(DiffY_fdGPH,order=c(4,0,5)))

fit123 <- fitted.values(arima(DiffY_fdGPH,order=c(1,0,2),include.mean = F))
...
sarima(DiffY_fdGPH,1,0,2,no.constant = T)
sarima(DiffY_fdGPH,4,0,5,details = T,no.constant = T)

################### ACF med fdGPH
res.arima.fdGPH <- residuals(arima(DiffY_fdGPH,order = c(1,0,2)))#p=1, q=2
