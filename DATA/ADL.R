  ...
  #vores nye tidsserie temp adjusteret for sæson
  temp_sason <- ts(model1temp$residuals)
  
  plot(decompose(ts(data_NO1[,3],frequency = 365))$random)
  
  
  auto.arima(hydro_sason,stepwise = F,approximation = F)#hydro
  # ARIMA (5,0,5) for hydro efter sæson adjusteret
  #prewhitning forsøg
  ###################
  
  aa_hydro <- auto.arima(hydro_sason,stepwise = F,approximation = F)#hydro
  ...
  #consump
  arma1model = auto.arima(data_NO1[,3],stepwise = F,approximation = F)
  ...
  aa_temp <- auto.arima(data_NO1[,4],stepwise = F,approximation = F)#temp
  ...
  aa_rain <- auto.arima(data_NO1[,5],stepwise = F,approximation = F)#rain
  ...
  arma11model = auto.arima(data_NO1[,1],stepwise = F,approximation = F)
  arma11model
