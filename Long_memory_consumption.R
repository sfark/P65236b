...
##########long memory for Hydro
plot(hydrodayli)
AIC_BIC_matrix_d <- c()
AIC_BIC_Resultat <- c()
for (i in 0:5) {
  for (j in 0:5) {
    AIC_value <-  AIC(fracdiff(hydrodayli
                               , nar = i, nma = j))
    BIC_value <- AIC(fracdiff(hydrodayli
                              , nar = i, nma = j),k = log(length(hydrodayli)) )#Ved K=log(length(X_t)), gives BIC.
    
    BIC_AIC_values <- cbind(AIC_value,BIC_value,i,j,fracdiff(hydrodayli,nar=i,nma=j)$d)
    AIC_BIC_Resultat <- rbind(AIC_BIC_Resultat,BIC_AIC_values)
    names <- c("AIC","BIC","p","q","d")
    colnames(AIC_BIC_Resultat) <- names
  }
