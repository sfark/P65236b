...
which(AIC_BIC_Resultat[,1] == min(AIC_BIC_Resultat[,1]), arr.ind = TRUE) #AIC, p=3,q=4
which(AIC_BIC_Resultat[,2] == min(AIC_BIC_Resultat[,2]), arr.ind = TRUE) #BIC, p=3,q=4
...
arfima(DiffY12,order = c(3,0,4),include.mean = F)
summary(arfima(DiffY12,c(3,0,4),lmodel = c("n"),dmean = F,itmean = F))  
{di_ACfF12 <- acf(fit34 ,plot = FALSE,lag.max = 50)
...
di_pACfF12 <- pacf(fit34 ,plot = FALSE,lag.max = 50)
